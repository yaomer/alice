...
void DB::sortCommand(Context& con)
{
    ...
    _sort(con, cmdlist[1], cmdops, by, des, offset, count, get);
    return;
syntax_err:
    con.append("-ERR syntax error\r\n");
}

void DB::_sort(Context& con, const String& key, unsigned cmdops, const String& by,
        ...
        const std::vector<String>& get)
{
    ...
    if (!(cmdops & SORT_ALPHA)) {
        ...
        if (cmdops & SORT_DESC)
            std::sort(result.begin(), result.end(), Alice::greater<String>());
        else
            std::sort(result.begin(), result.end(), Alice::less<String>());
    }
